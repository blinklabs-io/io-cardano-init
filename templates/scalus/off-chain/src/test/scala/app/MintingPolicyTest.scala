...
import scalus.cardano.ledger.{AddrKeyHash, AssetName, CardanoInfo}
...
/** Unit tests for the minting policy, run BOTH as a plain Scala function (debuggable, breakpoints
  * work) and as a compiled Plutus script on the built-in evaluator — no blockchain, milliseconds
  * per test.
  */
class MintingPolicyTest extends AnyFunSuite with ScalaCheckPropertyChecks with ScalusTest {
    ...
    test("should succeed when minted token name is correct and admin signature is correct") {
        ...
        // here you can use a debugger to step through the minting policy logic
        MintingPolicy.validate(config.toData)(ctx.toData)
        // run the minting policy script as a Plutus script
        assertEval(mintingScript.program $ ctx.toData, Success)
    }

    test("compiles to a non-empty Plutus V3 script") {
        val size = mintingScript.program.cborEncoded.length
        info(s"minting policy script size: $size bytes")
        assert(size > 0)
    }
